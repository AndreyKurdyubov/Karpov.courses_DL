...
def sinusoidal_embedding(n: int, d: int):
    # Позиционный эмбеддинг для времени
...
class UNetDiffusion(nn.Module):
    # Тот же UNet, но для времени добавляем синусоидальный эмбеддинг, для метки класса - просто Embedding
    ...
    def forward(self, x, t, y=None):
        # x -> (N, 2, 28, 28), изображение с позиционным эмбеддингом вдоль channel измерения
        t = self.time_embed(t)
        if self.is_y_cond:
            t += self.y_embed(y)
        n = len(x)
